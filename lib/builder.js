...
builder.schedule = function $schedule(repo, gitBranch, uuid, dockerOptions){
    ...
    // if(gitBranch === 'master') gitBranch = 'latest';
    ...
    git.clone(cloneUrl, path, gitBranch, logger).then(function(){
        ...
        try {
            ...
            logger.error('Seems like our target repo does not have a proper build.yml file');
        }
        ...
        console.log('--------------------------');
        console.log('project', JSON.stringify(projects, null, 4));
        var project;
...
builder.build = function $build(project, uuid, path, gitBranch, branch, dockerOptions){
    ...
    storage.saveBuild(uuid, buildId, project.id, branch, 'queued').then(function(){
        ...
        buildLogger.info('[%s] Updating build %s status as %s', buildId, 'passed');
