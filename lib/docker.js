...
docker.getAuth = function(buildId, registry, buildLogger){
    ...
    if(registry === 'dockerhub'){
        ...
        if(!options || !options.username || !options.email || !options.password){
            buildLogger.error('It seems that the build "%" should be pushed to the dockerhu', buildId);
            buildLogger.error('but you forgot to add your credentials in the config file "%s"', arg.config);
            buildLogger.error();
    ...
    // options.fromImage = 'registry.hub.docker.com/u/myname/myapp&tag=latest';
